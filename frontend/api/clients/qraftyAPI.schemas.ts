 ...
 * QRafty API
 * QRafty API helps to manage and create QR codes, allowing the user to create, edit and delete fully custom QR code    s.
...
export type ErrorModelDetailAnyOf = {
  code: string;
  reason: string;
};
...
export function isHTTPValidationError(
  ...
  /**
   * Type guard for checking if the error is of type HTTPValidationError
   *
   * @param error - The error object to be checked
   *
   * @returns boolean - True if the error is of type HTTPValidationError, False otherwise
   *
   */
  return (
    (error as HTTPValidationError).detail !== undefined &&
    Array.isArray((error as HTTPValidationError).detail)
  );
}

export function isErrorModel(error: unknown): error is ErrorModel {
  /**
   * Type guard for checking if the detail property of ErrorModel is of type ErrorModelDetail
   *
   * @param error - The error object to be checked
   *
   * @returns boolean - True if the detail property is of type ErrorModelDetail, False otherwise
   *
   */
  return (
    typeof (error as ErrorModel).detail === 'string' ||
    typeof (error as ErrorModel).detail === 'object'
  );
}
